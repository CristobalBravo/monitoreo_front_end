import { Component, ElementRef, OnInit, ViewChild, DoCheck, TemplateRef } from '@angular/core';
import {animate, AUTO_STYLE, state, style, transition, trigger} from '@angular/animations';
import {MenuItems} from '../../shared/menu-items/menu-items';
import { UsersService } from '../../services/users.service';
import Echo from 'laravel-echo';
import { NotificacionesServicesService } from '../../services/notificaciones-services.service';
...
export class AdminComponent implements OnInit, DoCheck {
  ...
  constructor(public menuItems: MenuItems, private userService:UsersService, private notificacionServices:NotificacionesServicesService
  ...
  websockets(){
    ...
    listen('NotificationEvent',(resp:any)=>{
      this.cantidad=resp.data.cantidad;
      this.notificaciones=resp.data.notificaciones;
    });
  }

  detalle(id, mensaje, vista, posicion){
    ...
    if(vista===0){
      ...
      this.notificacionServices.vistoNotificaction(formdata).subscribe((resp:any)=>{
        console.log(resp);
      });
    }

    let formdataNombre = new FormData();
    formdataNombre.append('mensaje', mensaje);
    this.notificacionServices.detalleNotificaction(formdataNombre).subscribe((resp:any)=>{
      this.modal.open(this.contenido);
      this.datosDetalle=resp.detalle;
      console.log(resp);
    })
    console.log(id);
  ...
  ignorar(){
    ...
    this.notificacionServices.ignorarNotificaction(formdata).subscribe((resp:any)=>{
      if(resp.code==200){
        this.modal.dismissAll();
        this.notificaciones.splice(this.posicion,1)
        if(this.cantidad>0){
